...
impl<I2C: I2c> Mpr121<I2C> {
    ...
    async fn write_register(&mut self, reg: Register, value: u8) -> Result<(), Mpr121Error> {
    ...
    async fn read_reg8(&mut self, reg: Register) -> Result<u8, Mpr121Error> {
        let mut val = [0u8];
        self.i2c
    ...
    #[maybe_async::maybe_async]
    //Reads the value, returns Err, if reading failed.
    async fn read_reg16(&mut self, reg: Register) -> Result<u16, Mpr121Error> {
        let mut val = [0u8, 0u8];
        self.i2c
            .write_read(self.addr.into(), &[reg.into()], &mut val)
            .await
